...
*/
/*
第一个优化函数：GlobalBundleAdjustemnt
第二个优化函数：BundleAdjustment
第三个优化函数：PoseOptimization
第四个优化函数：LocalBundleAdjustment
第五个优化函数：OptimizeEssentialGraph
第六个优化函数：OptimizeSim3
*/
#include "Optimizer.h"
        ...
        for (size_t i = 0; i < vpMP.size(); i++) {
            ...
            for (map<KeyFrame *, size_t>::const_iterator mit = observations.begin(); mit != observations.end(); mit++) {
                ...
                if (pKF->mvuRight[mit->second] < 0)//只有双目才有值，其余都是-1，属于Frame成员变量
                {
                    ...
                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(
                            id)));//VertexSBAPointXYZ
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(
                            pKF->mnId)));//VertexSE3Expmap
 ...
 */
    void Optimizer::OptimizeEssentialGraph(Map *pMap, KeyFrame *pLoopKF, KeyFrame *pCurKF,
        ...
        for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
            ...
            for (set<KeyFrame *>::const_iterator sit = sLoopEdges.begin(), send = sLoopEdges.end();
                 sit != send; sit++) {
                KeyFrame *pLKF = *sit;
