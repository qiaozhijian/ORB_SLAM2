...
namespace ORB_SLAM2 {
    ...
    bool LoopClosing::DetectLoop() {
        ...
        //根据最小score 计算出相连的候选关键帧们（候选关键帧是以前的帧）
        vector<KeyFrame *> vpCandidateKFs = mpKeyFrameDB->DetectLoopCandidates(mpCurrentKF, minScore);

        // If there are no loop candidates, just add new keyframe and return false
        //若无，则向mpKeyFrameDB添加关键帧，清除mvConsistentGroups，并可删除mpCurrentKF->SetErase()
        ...
        //循环计算每一个候选关键帧vpCandidateKFs的与其相连的关键帧，存入spCandidateGroup
        for (size_t i = 0, iend = vpCandidateKFs.size(); i < iend; i++) {
            ...
            //循环连续组（mvConsistentGroups），连续组由每个候选关键帧们组成，
            for (size_t iG = 0, iendG = mvConsistentGroups.size(); iG < iendG; iG++) {
                ...
                for (set<KeyFrame *>::iterator sit = spCandidateGroup.begin(), send = spCandidateGroup.end();
                     sit != send; sit++) {
                    //计算每一个”候选关键帧们“中是否存有“当前候选关键帧们”其中一个
                    if (sPreviousGroup.count(*sit)) {  //获取存有“当前候选关键帧们”其中一个的”候选关键帧们“的第二个参数，连续性参数
                        bConsistent = true;
                        bConsistentForSomeGroup = true;
                        break;
                    }
                }
                //如果存在则代表候选关键帧们互相具有连续性
                if (bConsistent) {
                    int nPreviousConsistency = mvConsistentGroups[iG].second;
                    int nCurrentConsistency = nPreviousConsistency + 1;
                    if (!vbConsistentGroup[iG]) {
                        //若当前一个的”候选关键帧们“没有添加过：将“当前候选关键帧们”存入vCurrentConsistentGroups，
                        //二个参数代表他与之前候选帧相连过
