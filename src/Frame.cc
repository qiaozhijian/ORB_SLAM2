 ...
 */
    vector<size_t> Frame::GetFeaturesInArea(const float &x, const float &y, const float &r, const int minLevel,
        ...
        const int nMinCellX = max(0, (int) floor(
                (x - mnMinX - r) * mfGridElementWidthInv));//floor  5.4 取5，此命令计算x点在哪个格子，跟0取最大
        if (nMinCellX >= FRAME_GRID_COLS)//超过则返回0个下面同理
            return vIndices;

        const int nMaxCellX = min((int) FRAME_GRID_COLS - 1,
                                  (int) ceil((x - mnMinX + r) * mfGridElementWidthInv));//ceil与floor相反
        if (nMaxCellX < 0)
            return vIndices;
        ...
        for (int ix = nMinCellX; ix <= nMaxCellX; ix++) {
            for (int iy = nMinCellY; iy <= nMaxCellY; iy++) {
                ...
                for (size_t j = 0, jend = vCell.size(); j < jend; j++) {
                    ...
                    if (bCheckLevels) {
                        if (kpUn.octave < minLevel)//代表是从金字塔哪一层提取的得到的数据。
                            continue;
                        if (maxLevel >= 0)
 ...
 */
    void Frame::ComputeStereoMatches() {
        mvuRight = vector<float>(N, -1.0f);//默认值-1
        ...
        vector<vector<size_t> > vRowIndices(nRows, vector<size_t>());  // 使用nRows个vector<size_t>()初始化

        for (int i = 0; i < nRows; i++)
            vRowIndices[i].reserve(200);
        // 每个vector<size_t>()包含200个空位
        const int Nr = mvKeysRight.size();  // 右相机特征点个数

        // 对于每一个右图的特征点，根据搜索范围确定其在左图匹配点的所在行的可能值，
        // 例如右图第一个特征点其y为200，因此198-202行上都放入id为1的值，供左图去匹配，
        // 左图特征点只需提供其Y值，即可确定右图范围内的所有候选特征点与其匹配
            ...
            if (bestDist < thOrbDist)  // 75
            {
                ...
                for (int incR = -L; incR <= +L; incR++) {
                    ...
                    IR = IR - IR.at<float>(w, w) *
                              cv::Mat::ones(IR.rows, IR.cols, CV_32F);//窗口中的每个元素减去正中心的那个元素，简单归一化，减小光照强度影响
